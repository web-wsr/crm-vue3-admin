...
async function handleEditRoleConfirm(row) {
  if (row.editRow.slug && row.editRow.name && row.editRow.description) {
    try {
      const res = await roleService.updateRole({ ...row.editRow, id: row.id });
      if (res.code === 200) {
        ElMessage({
          message: res.message,
          type: 'success'
...
async function handlePermissionConfirm() {
  try {
    const res = await roleService.assignRolePermissions({
      id: currentRoleId.value,
      permissions: permTree.value.getCheckedKeys()
    });
    if (res.code === 200) {
      ElMessage({
        message: res.message,
  ...
  <div class="container">
    <div class="app-container">
      ...
      <el-dialog v-model="showPermissonDialog" title="分配权限" width="700px">
        <el-tree ref="permTree" node-key="id" :data="permissionData" :props="{ label: 'name', children: 'children' }"
