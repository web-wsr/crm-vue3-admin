...
const userController = {
    ...
    login: async function(req,res,next){
        ...
        try{
            ...
            const user = users[0];
            console.log(user);
            // 通过user.id获取user_role表中的role_id
            ...
            if(state==='success'&& user){
                let token =JWT.sign({user_id:user.id,user_name:user.name,roleId:roleId,slug: slug},JWT_SECRET,{expiresIn:'30d'})
                console.log(token);
    ...
    getPermissions: async function(req, res, next) {
        try {
          ...
          const id= res.locals.userInfo.roleId
